...
class PlainMultiheadAttentionLoRA(nn.Module):
    def __init__(
        ...
        #self.in_proj_weight = nn.Parameter(existing_mha.in_proj_weight.clone())
        #self.in_proj_bias = nn.Parameter(existing_mha.in_proj_bias.clone())
        #self.out_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=existing_mha.out_proj.bias is not None)
        #self.out_proj_weight = nn.Parameter(existing_mha.out_proj.weight.clone())
        #self.out_proj_bias = nn.Parameter(existing_mha.out_proj.bias.clone())
        ...
        self.scaled_dot_product_attention = F._scaled_dot_product_attention
        ...
        """
        with torch.no_grad():
            self.in_proj_weight[:self.embed_dim, :].copy_(self.q_proj.weight.data)
            self.in_proj_weight[self.embed_dim:2*self.embed_dim, :].copy_(self.k_proj.weight.data)
            self.in_proj_weight[2*self.embed_dim:, :].copy_(self.v_proj.weight.data)
            if self.in_proj_bias is not None:
                self.in_proj_bias[:self.embed_dim].copy_(self.q_proj.bias.data)
                self.in_proj_bias[self.embed_dim:2*self.embed_dim].copy_(self.k_proj.bias.data)
                self.in_proj_bias[2*self.embed_dim:].copy_(self.v_proj.bias.data)
        
            self.out_proj.weight.copy_(self.proj.weight.data)
            self.out_proj.bias.copy_(self.proj.bias.data)
        """
    ...
    def forward_module(
        ...
        attn_output = self.scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, is_causal)[0]
