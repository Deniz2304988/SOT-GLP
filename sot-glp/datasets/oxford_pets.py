...
class OxfordPetsDataset(Dataset):
    ...
    def __init__(
        ...
        db = lib.load_json(os.path.join(abs_path_dirname, 'files', 'split_zhou_OxfordPets.json'))
        db = db[split]
        ...
        for x in db:
            self.paths.append(
                os.path.join(self.root, 'oxford-iiit-pet', 'images', x[0])
            )
