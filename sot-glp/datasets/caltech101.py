...
class Caltech101Dataset(Dataset):
    ...
    def __init__(
        ...
        db = lib.load_json(os.path.join(abs_path_dirname, 'files', 'split_zhou_Caltech101.json'))
        db = db[split]
        ...
        for x in db:
            self.paths.append(
                os.path.join(self.root, 'caltech101', '101_ObjectCategories', x[0])
            )
